...
public class SpikeEnemy extends Sprite {
    ...
    private void setRegAndPos(float dt) {
        setPosition(body.getPosition().x - getWidth() / 2, body.getPosition().y - getHeight() / 2 + 16 / game.GAME_PPM);//adding 5 for head spikes
    ...
    private void defineSpikeEnemy(float x, float y) {
        ...
        shape.setAsBox(20 / game.GAME_PPM, 30 / game.GAME_PPM);
    ...
    private void defineAnimation() {
        ...
        frames.clear();
        //fall
        frames.add(new TextureRegion(getTexture(), 13 * 64, 0, 64, 96));
        animations.put("enemyFall", new Animation(0.1f, frames));
        frames.clear();
        //idle
        for (int i = 0; i < 13; i++) {
            frames.add(new TextureRegion(getTexture(), i * 64, 96, 64, 96));
        }
        animations.put("enemyIdle", new Animation(0.1f, frames));
        frames.clear();
        //die
        for (int i = 0; i < 6; i++) {
            frames.add(new TextureRegion(getTexture(), i * 64, 192, 64, 96));
        }
